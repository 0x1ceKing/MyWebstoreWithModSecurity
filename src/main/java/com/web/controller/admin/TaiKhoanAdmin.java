...
//    }
@RequestMapping(value = {"/tai-khoan"}, method = RequestMethod.GET)
public String handleTaiKhoan(Model model,
                             @RequestParam(value = "keyword", required = false) String keyword) {
    Role userRole = Role.ROLE_USER;

    if (keyword != null && !keyword.isEmpty()) {
        // Nếu có keyword, tìm kiếm không phân biệt hoa thường
        model.addAttribute("taiKhoanList", userRepository.findByRoleAndKeywordIgnoreCase(userRole, keyword.toLowerCase()));
    } else {
        // Nếu không có keyword, hiển thị tất cả tài khoản ROLE_USER
        model.addAttribute("taiKhoanList", userRepository.findByRole(userRole));
    }
